...
class Cnn_Transformer(nn.Module):
    def __init__(self, num_emotions):
        ...
        self.fc1_linear = nn.Linear(808, num_emotions)  # 2344
...
if __name__ == '__main__':
    ...
    featuresFileName = 'features_{}_{}_Tracin.pkl'.format(FEATURES_TO_USE, impro_or_script)
    featuresExist = False
    toSaveFeatures = True
    # The storage location of IEMOCAP dataset
    WAV_PATH = "D:\Download\IEMOCAP/"
    wavpath1 = "D:\Pycharm\workplace\Tracin\\2secondwave\\traindata/"
    wavpath2 = "D:\Pycharm\workplace\Tracin\\2secondwave\\testdata/"
    ...
    MODEL_PATH = 'models/Tracin_second_epcho_32.pth'
    # data processing
    def process_data(path1,path2, t=2, train_overlap=1, val_overlap=1.6, RATE=16000, dataset='iemocap'):
        path1 = path1.rstrip('/')
        path2 = path2.rstrip('/')
        # wav_files = glob.glob(path + '/*.wav')
        wav_files_train = glob.glob(path1 + '/*.wav')
        wav_files_test = glob.glob(path2 + '/*.wav')
        ...
        # n是IEMOCAP数据集的长度
        # n = len(wav_files)
        n_1 = len(wav_files_train)
        n_2 = len(wav_files_test)
        ...
        # all_files_aug = []
        # 打乱IEMOCAP数据集里面的数据
        train_indices = list(np.random.choice(range(n_1), int(n_1), replace=False))
        valid_indices = list(np.random.choice(range(n_2), int(n_2), replace=False))
        # valid_indices = list(set(range(n))-set(train_indices))
        # 将打乱的数据集存放到对应的列表中
        for i in train_indices:
            train_files.append(wav_files_train[i])
            # train_files.append(wav_files_aug[i])
            # all_files_aug.append(wav_files_aug[i])
        for i in valid_indices:
            valid_files.append(wav_files_test[i])
        #
    ...
    if (featuresExist == True):
        ...
        train_X, train_y, val_dict = process_data(path1=wavpath1,path2=wavpath2)
        print(train_X.shape)
        print(len(val_dict))
